...
from .dataset import IterableImageDataset, ImageDataset, ImageDatasetV2
...
def create_dataset(
    ...
    # 增加按照txt读取方式
    elif name.startswith('DSMhands3'):
        txt_path = []
        if (is_training):
            train_txts = ["1208/train.txt", "keruisite/train.txt", "数据堂/train.txt", "正常类别/train.txt", "烟遮挡/train.txt", "背景/train.txt"]
            for t in train_txts:
                full_path = os.path.join(root, t)
                txt_path.append(full_path)
        else:
            val_txts = ["1208/val.txt", "keruisite/val.txt"]
            for t in val_txts:
                full_path = os.path.join(root, t)
                txt_path.append(full_path)
        ds = ImageDatasetV2(txt_path, flag=2)
    elif name.startswith('DSMhands4'):
        txt_path = []
        if (is_training):
            train_txts = ["1208/train.txt", "keruisite/train.txt", "数据堂/train.txt", "正常类别/train.txt"]
            for t in train_txts:
                full_path = os.path.join(root, t)
                txt_path.append(full_path)
        else:
            ...
            for t in val_txts:
                full_path = os.path.join(root, t)
                txt_path.append(full_path)
        ds = ImageDatasetV2(txt_path, flag=3)
    elif name.startswith('DSMhands5'):
        txt_path = []
        if (is_training):
            train_txts = ["1208/train.txt", "keruisite/train.txt", "数据堂/train.txt", "正常类别/train.txt"]
            for t in train_txts:
            ...
            for t in val_txts:
                full_path = os.path.join(root, t)
                txt_path.append(full_path)
        ds = ImageDatasetV2(txt_path, flag=4, is_training=is_training)
    else:
